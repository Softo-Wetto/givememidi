...
export default function AllMidiPage() {
    ...
    const fetchGenres = async () => {
      ...
      if (!error && data) {
        const unique = Array.from(
          new Set(data.map((r: any) => (r.genre || "").trim()).filter(Boolean))
  ...
  // Build base query with filters + multi-field search
  const buildQuery = () => {
    // pull only what you need (faster than select("*"))
    let query = supabase
      .from("music_files")
  ...
  // Fetch first page (reset list) whenever filters change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sentinelRef.current, midis.length, hasMore, loading, loadingMore]);
            ...
            <div className="flex flex-col md:flex-row gap-3 md:items-center">
              ...
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                ...
                >
                  ...
                  <option value="downloads" className="text-black">
                    Most downloaded
                  </option>
